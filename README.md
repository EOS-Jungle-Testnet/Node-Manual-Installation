...
```
mkdir /opt/bin
mkdir /opt/bin/v1.8.0
cp /opt/EOSIO/eos/build/programs/nodeos/nodeos /opt/bin/v1.8.0/
cp /opt/EOSIO/eos/build/programs/cleos/cleos /opt/bin/v1.8.0/
cp /opt/EOSIO/eos/build/programs/keosd/keosd /opt/bin/v1.8.0/
ln -sf /opt/bin/v1.8.0 /opt/bin/bin
```
...
https://github.com/EOSIO/eos/releases/tag/v1.8.0   
For example, for ubuntu 18.04 you need to download deb package eosio_1.8.0-1-ubuntu-18.04_amd64.deb        
To install it you can use apt:  
```
apt install ./eosio_1.8.0-1-ubuntu-18.04_amd64.deb   
```
It will download all dependencies and install EOSIO to /usr/opt/eosio/v1.8.0 
B. Copy binaries to keep old versions and make sym link to latest:  

```
 mkdir /opt/bin
 mkdir /opt/bin/v1.8.0
 cp /usr/opt/eosio/v1.8.0/bin/nodeos /opt/bin/v1.8.0/
 cp /usr/opt/eosio/v1.8.0/bin/cleos /opt/bin/v1.8.0/
 cp /usr/opt/eosio/v1.8.0/bin/keosd /opt/bin/v1.8.0/
 ln -sf /usr/opt/eosio/v1.8.0/bin /opt/bin/bin
```
